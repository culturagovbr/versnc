...
export class DetalheComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (Object.keys(this.entidade).length > 0) {
      if (this.entidade.cod_situacao_adesao == '6') {
        this.adesaoStatus = true;
      } else {
        this.adesaoStatus = false;
      }

      for (let comp in this.entidade.acoes_plano_trabalho) {
        let compKey = this.entidade.acoes_plano_trabalho[comp].key;
        let compValue = this.entidade.acoes_plano_trabalho[comp].value;
